...
public class ArmorEventHandler{
							...
							if (inf.getCooldown() > 0){
								if (props.getCooldown(inf.getRegistryName().toString()) < inf.getCooldown()){
									...
									if (player instanceof EntityPlayerMP)
										ArsMagicaReborn.proxy.blackoutArmorPiece((EntityPlayerMP)player, slot, inf.getCooldown());
								}
							}
						}
					}
	...
	private void doXPInfusion(EntityPlayer player, float xpMin, float xpMax){
		float amt = (float)((player.world.rand.nextFloat() * xpMin + (xpMax - xpMin)) * ArsMagicaReborn.config.getArmorXPInfusionFactor());
		...
		if (stack != null && stack.getItem() instanceof ItemArmor){
			...
			if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)){
				...
				for (String s : effects){
					event.getToolTip().add("-" + I18n.translateToLocal("am2.tooltip." + s.replaceAll(ArsMagicaReborn.MODID + ":", "")));
				}
