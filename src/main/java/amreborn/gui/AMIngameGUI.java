...
import amreborn.api.math.AMVector2;
...
//	private static final ResourceLocation inv_top = new ResourceLocation(ArsMagicaReborn.MODID, "textures/gui/Inventory_Top.png");
	private static final ResourceLocation mc_gui = new ResourceLocation("textures/gui/icons.png");
	private static final ResourceLocation spellbook_ui = new ResourceLocation(ArsMagicaReborn.MODID, "textures/gui/spellbook_ui.png");
...
//		itemRenderer = mc.getRenderItem();
	...
	public void renderGameOverlay(RenderGameOverlayEvent.Post e){
		...
		if (mc.currentScreen instanceof GuiHudCustomization || mc.inGameHasFocus) {
			...
			boolean drawAMHud = !ArsMagicaReborn.config.showHudMinimally() || (ci != null && (ci.getItem() == ItemDefs.spellBook || ci.getItem() == ItemDefs.spell || ci.getItem() == ItemDefs.arcaneSpellbook || ci.getItem() instanceof IBoundItem));
...
//			RenderBuffs(i, j);
	...
	private void RenderSpellBookUI(int i, int j, FontRenderer fontrenderer, ItemStack bookStack){
		...
		AMVector2 spellbookVec = getShiftedVector(ArsMagicaReborn.config.getSpellBookPosition(), i, j);
	...
	private void RenderManaBar(int i, int j, FontRenderer fontRendererObj){
		...
		AMVector2 Burnout_hud = getShiftedVector(ArsMagicaReborn.config.getBurnoutHudPosition(), i, j);
		AMVector2 mana_hud = getShiftedVector(ArsMagicaReborn.config.getManaHudPosition(), i, j);
		...
		if (ArsMagicaReborn.config.showHudBars()){
			...
			AMVector2 magicLevelPos = getShiftedVector(ArsMagicaReborn.config.getLevelPosition(), i, j);
		...
		if (ArsMagicaReborn.config.getShowNumerics()){
			GlStateManager.enableBlend();
			String spellcost = "";
			ItemStack curItem = Minecraft.getMinecraft().player.getHeldItem(EnumHand.MAIN_HAND);
			if (curItem != null && (curItem.getItem() == ItemDefs.spell
			...
			AMVector2 manaNumericPos = getShiftedVector(ArsMagicaReborn.config.getManaNumericPosition(), i, j);
			AMVector2 burnoutNumericPos = getShiftedVector(ArsMagicaReborn.config.getBurnoutNumericPosition(), i, j);
...
//	}
	...
	private void RenderArmorStatus(int i, int j, Minecraft mc, FontRenderer fontRenderer){
		if (!ArsMagicaReborn.config.showArmorUI())
			return;
...
//				}
		...
		mc.renderEngine.bindTexture(new ResourceLocation(ArsMagicaReborn.MODID, "textures/gui/overlay.png"));
		AMVector2 shieldPos = getShiftedVector(ArsMagicaReborn.config.getManaShieldingPosition(), scaledResolution.getScaledWidth(), scaledResolution.getScaledHeight());
	...
	private AMVector2 getArmorSlotPosition(int slot, int screenWidth, int screenHeight){
		switch (slot){
		case 0:
			return getShiftedVector(ArsMagicaReborn.config.getArmorPositionHead(), screenWidth, screenHeight);
		case 1:
			return getShiftedVector(ArsMagicaReborn.config.getArmorPositionChest(), screenWidth, screenHeight);
		case 2:
			return getShiftedVector(ArsMagicaReborn.config.getArmorPositionLegs(), screenWidth, screenHeight);
		case 3:
			return getShiftedVector(ArsMagicaReborn.config.getArmorPositionBoots(), screenWidth, screenHeight);
		}
		return new AMVector2(0, 0);
	}

	public void RenderAffinity(int i, int j){
		AMVector2 affinityPos = getShiftedVector(ArsMagicaReborn.config.getAffinityPosition(), i, j);
	...
	public void RenderContingency(int i, int j){
		...
		AMVector2 contingencyPos = getShiftedVector(ArsMagicaReborn.config.getContingencyPosition(), i, j);
		Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
		TextureAtlasSprite icon = null;
		ContingencyType type = EntityExtension.For(Minecraft.getMinecraft().player).getContingencyType();
		switch (type){
		case DAMAGE:
			icon = SpellIconManager.INSTANCE.getSprite(ArsMagicaReborn.MODID + ":contingency_damage");
			break;
		case FALL:
			icon = SpellIconManager.INSTANCE.getSprite(ArsMagicaReborn.MODID + ":contingency_fall");
			break;
		case HEALTH:
			icon = SpellIconManager.INSTANCE.getSprite(ArsMagicaReborn.MODID + ":contingency_health");
			break;
		case FIRE:
			icon = SpellIconManager.INSTANCE.getSprite(ArsMagicaReborn.MODID + ":contingency_fire");
			break;
		case DEATH:
			icon = SpellIconManager.INSTANCE.getSprite(ArsMagicaReborn.MODID + ":contingency_death");
			break;
		case NULL:
		default:
			return;
...
//
//		if (!ArsMagicaReborn.config.getShowBuffs()){
//			return;
//		}
//
//		int barWidth = i / 8;
//
//		AMVector2 posBuffStart = getShiftedVector(ArsMagicaReborn.config.getPositiveBuffsPosition(), i, j);
//		AMVector2 negBuffStart = getShiftedVector(ArsMagicaReborn.config.getNegativeBuffsPosition(), i, j);
//
...
//	}
	...
	public void RenderMagicXP(int i, int j){
		...
		if (props.getCurrentLevel() > 0){
			GlStateManager.enableBlend();
			AMVector2 position = getShiftedVector(ArsMagicaReborn.config.getXPBarPosition(), i, j);
			AMVector2 dimensions = new AMVector2(182, 5);
			Minecraft.getMinecraft().renderEngine.bindTexture(mc_gui);
			GlStateManager.color(0.5f, 0.5f, 1.0f, ArsMagicaReborn.config.showXPAlways() ? 1.0f : AMGuiHelper.instance.getMagicXPBarAlpha());
			...
			if (ArsMagicaReborn.config.getShowNumerics() && (ArsMagicaReborn.config.showXPAlways() || AMGuiHelper.instance.getMagicXPBarAlpha() > 0)){
				String xpStr = I18n.translateToLocal("am2.gui.xp") + ": " + +(int)(props.getCurrentXP() * 100) + "/" + (int)(props.getMaxXP() * 100);
				AMVector2 numericPos = getShiftedVector(ArsMagicaReborn.config.getXPNumericPosition(), i, j);
