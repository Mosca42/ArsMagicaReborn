...
import amreborn.gui.controls.GuiButtonSkillTree;
...
//		}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(ArsMagicaReborn.MODID, "textures/occulus/overlay.png"));
		//Overlay
		...
		float calcXOffest = ((float)offsetX / 568) * (1 - renderRatio);
		{
			...
			zLevel = 0F;
			Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(ArsMagicaReborn.MODID, "textures/occulus/skill_points.png"));
		...
		if (currentTree != SkillDefs.TREE_AFFINITY) {
			...
			for (Skill s : skills) {
				...
				if (ArsMagicaReborn.disabledSkills.isSkillDisabled(s.getID()))
						...
						if (ability.getAffinity() == aff) {
							...
							drawString.add(TextFormatting.RESET.toString()
									...
									+ I18n.translateToLocal("affinityability."
											+ ability.getRegistryName().toString().replaceAll(ArsMagicaReborn.MODID + ":", "")
