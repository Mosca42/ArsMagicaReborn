...
public class AMGuiHelper {
	...
	public static void DrawIconAtXY(TextureAtlasSprite IIcon, float x, float y, float zLevel, int w, int h, int color) {
		...
		tessellator.getBuffer().pos(x, y + h, zLevel).tex(IIcon.getMinU(), IIcon.getMaxV()).color(RenderUtils.getRed(color), RenderUtils.getGreen(color), RenderUtils.getBlue(color), 1.0f).endVertex();
		tessellator.getBuffer().pos(x + w, y + h, zLevel).tex(IIcon.getMaxU(), IIcon.getMaxV()).color(RenderUtils.getRed(color), RenderUtils.getGreen(color), RenderUtils.getBlue(color), 1.0f).endVertex();
		tessellator.getBuffer().pos(x + w, y, zLevel).tex(IIcon.getMaxU(), IIcon.getMinV()).color(RenderUtils.getRed(color), RenderUtils.getGreen(color), RenderUtils.getBlue(color), 1.0f).endVertex();
		tessellator.getBuffer().pos(x, y, zLevel).tex(IIcon.getMinU(), IIcon.getMinV()).color(RenderUtils.getRed(color), RenderUtils.getGreen(color), RenderUtils.getBlue(color), 1.0f).endVertex();
	...
	public static void shiftView(float f) {
		...
		if (flip > 0) {
			...
			// GlStateManager.translate(0, (entity.eyeHeight * (smoothedFlip / 180f) -
			// 0.1f), 0);
			GlStateManager.rotate(lastFlip + (flip - lastFlip) * f, 0, 0, 1);
			// flipView(f);
		}
	...
	public static boolean overrideMouseInput(EntityRenderer renderer, float f, boolean b) {
		...
		if (mc.gameSettings.smoothCamera) {
			...
			mc.player.setLocationAndAngles(mc.player.getPosition().getX(), mc.player.getPosition().getY(), mc.player.getPosition().getZ(), -f3, f4 * (float) b0);
		} else {
			mc.player.setLocationAndAngles(mc.player.getPosition().getX(), mc.player.getPosition().getY(), mc.player.getPosition().getZ(), -f3, f4 * (float) b0);
		}
