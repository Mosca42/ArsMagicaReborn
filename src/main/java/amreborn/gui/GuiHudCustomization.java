...
public class GuiHudCustomization extends GuiScreen {
	...
	public void initGui() {
		...
		doShowBuffs = ArsMagicaReborn.config.getShowBuffs();
		doShowNumerics = ArsMagicaReborn.config.getShowNumerics();
		doShowHudMinimally = ArsMagicaReborn.config.showHudMinimally();
		doShowArmor = ArsMagicaReborn.config.showArmorUI();
		doShowXPAlways = ArsMagicaReborn.config.showXPAlways();
		doShowBars = ArsMagicaReborn.config.showHudBars();
		...
		spellBook = new GuiButtonVariableDims(21, 0, 0, I18n.translateToLocal("item." + ArsMagicaReborn.MODID + ":spellBook.name")).setBorderOnly(true).setDimensions(106, 15);
		...
		initButtonAndSnapData(manaButton, ArsMagicaReborn.config.getManaHudPosition());
		initButtonAndSnapData(burnoutButton, ArsMagicaReborn.config.getBurnoutHudPosition()); // new AMVector2(0.5, 0.5)
		initButtonAndSnapData(levelButton, ArsMagicaReborn.config.getLevelPosition());
		initButtonAndSnapData(affinityButton, ArsMagicaReborn.config.getAffinityPosition());
		initButtonAndSnapData(positiveBuffs, ArsMagicaReborn.config.getPositiveBuffsPosition());
		initButtonAndSnapData(negativeBuffs, ArsMagicaReborn.config.getNegativeBuffsPosition());
		initButtonAndSnapData(armorHead, ArsMagicaReborn.config.getArmorPositionHead());
		initButtonAndSnapData(armorChest, ArsMagicaReborn.config.getArmorPositionChest());
		initButtonAndSnapData(armorLegs, ArsMagicaReborn.config.getArmorPositionLegs());
		initButtonAndSnapData(armorBoots, ArsMagicaReborn.config.getArmorPositionBoots());
		initButtonAndSnapData(xpBar, ArsMagicaReborn.config.getXPBarPosition());
		initButtonAndSnapData(contingency, ArsMagicaReborn.config.getContingencyPosition());
		initButtonAndSnapData(manaNumeric, ArsMagicaReborn.config.getManaNumericPosition());
		initButtonAndSnapData(burnoutNumeric, ArsMagicaReborn.config.getBurnoutNumericPosition());
		initButtonAndSnapData(XPNumeric, ArsMagicaReborn.config.getXPNumericPosition());
		initButtonAndSnapData(spellBook, ArsMagicaReborn.config.getSpellBookPosition());
		initButtonAndSnapData(manaShielding, ArsMagicaReborn.config.getManaShieldingPosition());
	...
	private void storeGuiPositions() {
		ArsMagicaReborn.config.setGuiPositions(getSnapVector(manaButton), getSnapVector(burnoutButton), getSnapVector(levelButton), getSnapVector(affinityButton), getSnapVector(positiveBuffs), getSnapVector(negativeBuffs), getSnapVector(armorHead), getSnapVector(armorChest), getSnapVector(armorLegs), getSnapVector(armorBoots), getSnapVector(xpBar), getSnapVector(contingency), getSnapVector(manaNumeric), getSnapVector(burnoutNumeric), getSnapVector(XPNumeric), getSnapVector(spellBook), getSnapVector(manaShielding), doShowBuffs, doShowNumerics, doShowHudMinimally, doShowArmor, doShowXPAlways, doShowBars);
	}
	...
	protected void keyTyped(char par1, int par2) throws IOException {
		if (par2 == Keyboard.KEY_ESCAPE) {
			ArsMagicaReborn.config.saveGuiPositions();
			if (showOptions) {
				showOptions = false;
				setOptionsVisibility(false);
				return;
