...
import amreborn.gui.controls.GuiButtonCompendiumLink;
import amreborn.gui.controls.GuiButtonCompendiumNext;
import amreborn.gui.controls.GuiButtonCompendiumTab;
import amreborn.gui.controls.GuiSpellImageButton;
...
public class GuiCompendiumIndex extends GuiScreen{
	...
	private static final ResourceLocation background = new ResourceLocation(ArsMagicaReborn.MODID, "textures/gui/arcanecompendiumindexgui.png");

	public GuiCompendiumIndex(){
		categories = CompendiumCategory.getCategories();
		currentCategory = categories.iterator().next();
		lines = new ArrayList<String>();
		String path = amreborn.lore.ArcaneCompendium.For(Minecraft.getMinecraft().player).getPath();
		if (path != null) {
...
//				}
	...
	private void switchCategory(CompendiumCategory category) {
		...
		amreborn.lore.ArcaneCompendium.For(Minecraft.getMinecraft().player).setPath(category.getID());
	}
