...
import amreborn.codechicken.LightningBoltCommon.Segment;
...
public class LightningBolt extends Particle{
	...
	private static AMVector3 getRelativeViewVector(AMVector3 pos){
		EntityPlayer renderentity = ArsMagicaReborn.proxy.getLocalPlayer();
...
//				tessellator.setColorRGBA_F(this.particleRed, this.particleGreen, this.particleBlue, mainalpha * rendersegment.light);
	...
	public void renderParticle(VertexBuffer worldRendererIn, Entity entityIn, float partialTicks, float cosyaw, float cospitch, float sinyaw, float sinsinpitch, float cossinpitch) {
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(ArsMagicaReborn.MODID, "textures/items/particles/smoke.png"));
