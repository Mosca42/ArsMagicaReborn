...
public class AMParticleIcons{
	...
	private void loadAndInitIcon(String name, String TextureAtlasSpritePath, TextureMap register, boolean registerName){
		register.registerSprite(new ResourceLocation(ArsMagicaReborn.MODID + ":items/particles/" + TextureAtlasSpritePath));
		TextureAtlasSprite icon = register.getTextureExtry(new ResourceLocation(ArsMagicaReborn.MODID + ":items/particles/" + TextureAtlasSpritePath).toString());
		if (registerName){
			if (icon != null){
				icons.put(name, icon);
			}
