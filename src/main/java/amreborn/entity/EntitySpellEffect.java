...
import amreborn.particles.ParticleFleePoint;
import amreborn.particles.ParticleFloatUpward;
import amreborn.particles.ParticleOrbitPoint;
import amreborn.spell.modifier.Colour;
import amreborn.utils.AMLineSegment;
...
public class EntitySpellEffect extends Entity{
	...
	private void zoneUpdate(){
		if (this.world.isRemote){
			if (!ArsMagicaReborn.config.NoGFX()){
				...
				if ((ArsMagicaReborn.config.FullGFX() && this.ticksExisted % 2 == 0) || this.ticksExisted % 8 == 0){
					for (int i = 0; i < 4; ++i){
						...
						AMParticle effect = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, AMParticleDefs.getParticleForAffinity(AffinityShiftUtils.getMainShiftForStack(spellStack)), x, posY, z);
						if (effect != null){
							...
							if (ArsMagicaReborn.config.LowGFX()){
	...
	private void rainOfFireUpdate(){
		...
		if (world.isRemote){
			...
			for (int i = 0; i < 10; ++i){
				...
				AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "explosion_2", x, y, z);
				if (particle != null){
			...
			//TODO: SoundHelper.instance.loopSound(worldObj, (float)posX, (float)posY, (float)posZ, ArsMagicaReborn.MODID + ":spell.loop.fire", 1.0f);
		}else{
	...
	private void blizzardUpdate(){
		...
		if (world.isRemote){
			...
			for (int i = 0; i < 20; ++i){
				...
				AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "snowflakes", x, y, z);
				if (particle != null){
					particle.setMaxAge(20);
			...
			AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "smoke", x, y, z);
			if (particle != null){
			...
			//TODO: SoundHelper.instance.loopSound(worldObj, (float)posX, (float)posY, (float)posZ, ArsMagicaReborn.MODID + ":spell.loop.air", 1.0f);
		}else{
			List<Entity> possibleTargets = world.getEntitiesWithinAABB(EntityLivingBase.class, new AxisAlignedBB(posX - radius, posY - 1, posZ - radius, posX + radius, posY + 3, posZ + radius));
			for (Entity e : possibleTargets){
				if (e != dummycaster){
	...
	private void wallUpdate(){
		if (world.isRemote){
			...
			for (float i = 0; i < dist; i += 0.5f){
				...
				effect = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, AMParticleDefs.getParticleForAffinity(AffinityShiftUtils.getMainShiftForStack(spellStack)), x, posY, z);
				if (effect != null){
					effect.setIgnoreMaxAge(false);
					effect.addRandomOffset(1, 1, 1);
					effect.setMaxAge(20);
