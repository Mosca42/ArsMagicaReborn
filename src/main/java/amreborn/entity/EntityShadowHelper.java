...
import amreborn.entity.ai.EntityAISpellmaking;
import amreborn.particles.AMParticle;
import amreborn.particles.ParticleFloatUpward;
import amreborn.proxy.ShadowSkinHelper;
...
public class EntityShadowHelper extends EntityLiving{
	...
	private void spawnParticles(){
		if (world.isRemote){
			for (int i = 0; i < 25 * ArsMagicaReborn.config.getGFXLevel() + 1; ++i){
				AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "arcane", posX, posY, posZ);
				if (particle != null){
