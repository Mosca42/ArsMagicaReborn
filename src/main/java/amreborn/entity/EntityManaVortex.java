...
import amreborn.particles.ParticleArcToEntity;
		...
		if (getTicksToExist() - this.ticksExisted <= 5 && !hasGoneBoom){
			...
			if (!world.isRemote){
				...
				for (int i = 0; i < 360; i += ArsMagicaReborn.config.FullGFX() ? 5 : ArsMagicaReborn.config.LowGFX() ? 10 : 20){
					AMParticle effect = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "ember", this.posX, this.posY, this.posZ);
					if (effect != null){
		...
		if (getTicksToExist() - this.ticksExisted > 30){
			...
			for (Object o : playerArray){
				...
				if (world.isRemote){
					...
					if (ArsMagicaReborn.config.LowGFX() && (this.ticksExisted % 4) != 0){
						break;
					}
					AMParticle effect = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "ember", e.posX, e.posY + (e.getEyeHeight() / 2), e.posZ);
					if (effect != null){
						effect.setRGBColorF(0.24f, 0.24f, 0.8f);
