...
import amreborn.entity.ai.EntityAIChestDeposit;
import amreborn.entity.ai.EntityAIPickup;
import amreborn.entity.ai.EntityAITargetNearbyInanimate;
...
public class EntityBroom extends EntityCreature{
	...
	public void onUpdate(){
		if (world.isRemote){
			updateRotations();
			if (isMoving()){
				AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "smoke", posX, posY, posZ);
				if (particle != null){
	...
	public EnumActionResult applyPlayerInteraction(EntityPlayer player, Vec3d vec, EnumHand hand){
		if (player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() == ItemDefs.spellStaffMagitech){
			if (this.world.isRemote){
				for (int i = 0; i < ArsMagicaReborn.config.getGFXLevel() * 2; ++i){
					AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "smoke", posX, posY, posZ);
					if (particle != null){
