...
import amreborn.particles.ParticleColorShift;
	...
	public void onUpdate(){
		...
		if (world.isRemote){
			if (getIsMoonstoneMeteor()){
				AMParticle fire = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "explosion_2", posX, posY, posZ);
				if (fire != null){
					fire.setMaxAge(20);
				...
				for (float i = 0; i < Math.abs(motionY); i += 0.1f){
					AMParticle star = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "ember", posX + motionX * i, posY + motionY * i, posZ + motionZ * i);
					if (star != null){
						star.setMaxAge(22);
		...
		if (getIsShootingStar()){
			...
			if (movingobjectposition.entityHit != null && movingobjectposition.entityHit instanceof EntityLivingBase){
				...
				if (this.getIsMoonstoneMeteor()){
					...
					this.world.newExplosion(this, this.target.xCoord, this.target.yCoord, this.target.zCoord, 0.8f, false, ArsMagicaReborn.config.moonstoneMeteorsDestroyTerrain());
