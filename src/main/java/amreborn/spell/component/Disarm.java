...
public class Disarm extends SpellComponent{	
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target){
		...
		if (target instanceof EntityPlayer && (!ArsMagicaReborn.config.getDisarmAffectsPlayers() || (!world.isRemote && !FMLCommonHandler.instance().getMinecraftServerInstance().isPVPEnabled())))
			return false;
		...
		if (target instanceof EntityPlayer && ((EntityPlayer)target).getHeldItemOffhand() != null && !target.world.isRemote && (rnd.nextInt(9) + 1 <= damage)
			...
			world.spawnEntity(item);
			ItemStack stack2 = player.inventory.offHandInventory.get(0);
