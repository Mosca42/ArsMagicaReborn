...
public class MeltArmor extends SpellComponent{
	...
	private void doMeltArmor(EntityLivingBase caster, NonNullList<ItemStack> armorInventory){
		double mmpsCharge = getMMPSCharge(armorInventory);
	...
	private double getMMPSCharge(NonNullList<ItemStack> armorInventory){
		double total = -1;
		for (ItemStack stack : armorInventory){
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier){
		AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "radiant", x + 0.5, y + 0.5, z + 0.5);
		if (particle != null){
