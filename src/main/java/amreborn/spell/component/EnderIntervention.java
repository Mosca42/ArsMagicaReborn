...
public class EnderIntervention extends SpellComponent{
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target){
		...
		if (target.dimension == 1){
			...
			ArsMagicaReborn.proxy.addDeferredDimensionTransfer((EntityLivingBase)target, -1);
		}
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier){
		for (int i = 0; i < 100; ++i){
			AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "ghost", x, y - 1, z);
			if (particle != null){
