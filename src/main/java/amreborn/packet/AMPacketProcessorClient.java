...
import amreborn.particles.ParticleChangeSize;
...
//				break;
			case AMPacketIDs.PARTICLE_SPAWN_SPECIAL:
				ArsMagicaReborn.proxy.particleManager.handleClientPacketData(Minecraft.getMinecraft().world, remaining);
				break;
...
//
	private void handleRcvPowerPaths(byte[] data){
		...
		if (bite == 0){
			...
			ArsMagicaReborn.proxy.receivePowerPathVisuals(pnd.getNodePaths());
		}else if (bite == 1){
			int x = rdr.getInt();
			int y = rdr.getInt();
			int z = rdr.getInt();
			ArsMagicaReborn.proxy.setTrackedPowerCompound((NBTTagCompound)compound.copy());
...
//
	private void handleStarFall(byte[] data){
		...
		for (int i = 0; i < 360; i += ArsMagicaReborn.config.FullGFX() ? 5 : ArsMagicaReborn.config.LowGFX() ? 10 : 20){
			AMParticle effect = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(Minecraft.getMinecraft().world, "sparkle2", x, y + 1.5, z);
			if (effect != null){
...
//		Minecraft.getMinecraft().world.playSound(x, y, z, ArsMagicaReborn.MODID + ":spell.special.starfall", 2.0f, 1.0f, false);
	}
//
...
//
	private void handlePlayerLoginData(byte[] data, EntityPlayer player){
		...
		ArsMagicaReborn.config.setSkillTreeSecondaryTierCap(skillTreeLock);
		int[] disabledSkills = rdr.getIntArray();
		double manaCap = rdr.getDouble();

		ArsMagicaReborn.config.setManaCap(manaCap);
		...
		ArsMagicaReborn.disabledSkills.disableAllSkillsIn(disabledSkills);
	}
