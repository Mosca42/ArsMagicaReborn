package amreborn.api.compendium.pages;
...
public class PageItemStack extends CompendiumPage<ItemStack> {
	...
	protected void renderPage(int posX, int posY, int mouseX, int mouseY) {	
		...
		mc.renderEngine.bindTexture(new ResourceLocation(ArsMagicaReborn.MODID, "textures/gui/arcanecompendiumguiextras.png"));
		zLevel++;
	...
	private void RenderRecipe(int cx, int cy, int mousex, int mousey){
		...
		if (element != null){
			...
			if (element.getCount() > 1)
				mc.fontRendererObj.drawString("x" + element.getCount(), sx + 16, sy + 8, 0, false);
...
//		List<ItemStack> oredict = OreDictionary.getOres(stack.getItem().getUnlocalizedName());
		NonNullList<ItemStack> alternates = NonNullList.create();
