...
import amreborn.utils.CloakUtils;
...
public class EntityHandler {
	...
	public void onMouseEvent(MouseEvent event){
		event.setCanceled(ArsMagicaReborn.proxy.setMouseDWheel(event.getDwheel()));
	}
	...
	public void attachEntity(AttachCapabilitiesEvent.Entity event) {
		if (event.getEntity() instanceof EntityLivingBase) {
			...
			if (event.getEntity() instanceof EntityPlayer) {
				...
				event.addCapability(new ResourceLocation(ArsMagicaReborn.MODID, "Compendium"), compendium);
				event.addCapability(SkillData.ID, skillData);
				event.addCapability(AffinityData.ID, affData);
				event.addCapability(new ResourceLocation(ArsMagicaReborn.MODID, "RiftStorage"), storage);
			}
		}
	}
	...
	public void onEntityInteract(EntityInteract event){
		if (event.getTarget() instanceof EntityItemFrame){
			ArsMagicaReborn.proxy.itemFrameWatcher.startWatchingFrame((EntityItemFrame)event.getTarget());
		}
	}
	...
	public void renderTick (RenderGameOverlayEvent event) {
		Minecraft.getMinecraft().mcProfiler.startSection("ArsMagicaReborn-Overlay");
		GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
		GL11.glPushMatrix();
		if (event.getType() == ElementType.CROSSHAIRS)
			ArsMagicaReborn.proxy.renderGameOverlay();
		GL11.glPopMatrix();
		GL11.glPopAttrib();
	...
	public void entityDeath(LivingDeathEvent e) {
		...
		if (e.getEntityLiving() instanceof EntityPlayer){
			ArsMagicaReborn.proxy.playerTracker.onPlayerDeath((EntityPlayer)e.getEntityLiving());
		}
	...
	public void onPlayerRender(RenderPlayerEvent.Pre event){
		...
		if (!ArsMagicaReborn.proxy.playerTracker.hasCLS(event.getEntityPlayer().getUniqueID().toString())){
