...
import amreborn.api.sources.DamageSourceFire;
import amreborn.api.sources.DamageSourceFrost;
import amreborn.bosses.ai.EntityAICastSpell;
import amreborn.bosses.ai.EntityAISmash;
import amreborn.bosses.ai.EntityAIStrikeAttack;
import amreborn.bosses.ai.EntityWinterGuardianLaunchArm;
import amreborn.bosses.ai.ISpellCastCallback;
...
import amreborn.particles.ParticleFleeEntity;
...
public class EntityWinterGuardian extends AM2Boss{
	...
	public void onUpdate(){
		if (world.getBiome(getPosition()).getEnableSnow() && world.getWorldInfo().isRaining()){
			if (world.isRemote){
				AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "ember", posX + (rand.nextFloat() * 6 - 3), posY + 2 + (rand.nextFloat() * 2 - 1), posZ + (rand.nextFloat() * 6 - 3));
				if (particle != null){
	...
	private void spawnParticles(){
		for (int i = 0; i < ArsMagicaReborn.config.getGFXLevel() * 4; ++i){
			int rnd = rand.nextInt(10);
			AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, rnd < 5 ? "snowflakes" : "ember", posX + (rand.nextFloat() * 0.4 - 0.2), posY + 2, posZ + (rand.nextFloat() * 0.4 - 0.2));
			if (particle != null){
				if (rnd < 2 || rnd > 8){
