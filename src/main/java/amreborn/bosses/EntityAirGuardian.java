...
import amreborn.api.sources.DamageSourceLightning;
import amreborn.bosses.ai.EntityAIDispel;
import amreborn.bosses.ai.EntityAIHurricane;
import amreborn.bosses.ai.EntityAISpawnWhirlwind;
import amreborn.defs.AMSounds;
import amreborn.defs.ItemDefs;
import amreborn.entity.ai.EntityAIGuardSpawnLocation;
	...
	public void onUpdate(){
		...
		case SPINNING:
			...
			if (this.world.isRemote){
				for (int i = 0; i < ArsMagicaReborn.config.getGFXLevel(); ++i){
					AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "wind", posX + world.rand.nextDouble() * 4 - 2, posY, posZ + world.rand.nextDouble() * 4 - 2);
					if (particle != null){
		...
		if (this.posY < 150){
			if (world.isRemote){
				for (int i = 0; i < 25; ++i){
					AMParticle wind = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "wind", posX, posY, posZ);
					if (wind != null){
