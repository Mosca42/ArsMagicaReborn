...
import amreborn.bosses.ai.EntityAIDive;
import amreborn.bosses.ai.EntityAIFireRain;
import amreborn.bosses.ai.EntityAIFlamethrower;
...
public class EntityFireGuardian extends AM2Boss{
	...
	private void nova(){
		if (this.world.isRemote){
			for (int i = 0; i < 36; ++i){
				AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "explosion_2", posX, posY - 3, posZ);
				if (particle != null){
	...
	private void flamethrower(){
		...
		if (world.isRemote){
			AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "explosion_2", posX + (Math.cos(Math.toRadians(this.renderYawOffset + 90)) * 2), posY + 3, posZ + (Math.sin(Math.toRadians(this.renderYawOffset + 90)) * 2));
			if (particle != null){
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2){
		...
		if (par1DamageSource.isFireDamage()){
			this.heal(par2);
			if (this.world.isRemote){
				AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "sparkle", posX, posY - 1, posZ);
				if (particle != null){
