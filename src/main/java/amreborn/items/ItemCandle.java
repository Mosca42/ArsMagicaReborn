...
public class ItemCandle extends ItemArsMagica{
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (!player.getHeldItem(hand).hasTagCompound() || !player.getHeldItem(hand).getTagCompound().hasKey("search_block")){
			...
			if (player.isSneaking() && block != null && block.getBlockHardness(worldIn, pos) > 0f && worldIn.getTileEntity(pos) == null){
				if (!worldIn.isRemote){
					setSearchBlock(block, player.getHeldItem(hand));
					worldIn.setBlockToAir(pos);
				}else{
					AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(worldIn, "radiant", pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5);
					if (particle != null){
		...
		if (!worldIn.isRemote){
			...
			if (player.getHeldItem(hand).hasTagCompound() && player.getHeldItem(hand).getTagCompound().hasKey("search_block")){
				player.sendMessage(new TextComponentString(I18n.translateToLocal("am2.tooltip.candlecantplace")));
...
//	}
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int indexInInventory, boolean isCurrentlyHeld){
		if (entity instanceof EntityPlayer){
			...
			if (isCurrentlyHeld && !world.isRemote && ArsMagicaReborn.config.candlesAreRovingLights() && world.isAirBlock(player.getPosition()) && world.getLightFor(EnumSkyBlock.BLOCK, player.getPosition()) < 14){
