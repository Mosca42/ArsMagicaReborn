...
public class ItemSpellStaff extends ItemArsMagica {
	...
	public String getItemStackDisplayName(ItemStack par1ItemStack) {
		if (isMagiTechStaff()) {
			return I18n.translateToLocal("item."+ ArsMagicaReborn.MODID +":spell_staff_magitech.name");
		}
				...
				if (te != null && te instanceof IPowerNode) {
					if (player.isSneaking()) {
						...
						for (PowerTypes type : types) {
							...
							player.sendMessage(new TextComponentString(String.format(I18n.translateToLocal("am2.tooltip.det_eth"), type.getChatColor(), type.name(), String.format("%.2f", power))));
						}
					}
					return EnumActionResult.SUCCESS;
				}
	...
	private ItemStack getSpellStack(ItemStack staffStack) {
		...
		stack.deserializeNBT(staffStack.getTagCompound().getCompoundTag(NBT_SPELL));
		return stack;
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer entityplayer, EnumHand hand) {
		if (isMagiTechStaff())
			return new ActionResult<ItemStack>(EnumActionResult.PASS, entityplayer.getHeldItem(hand));

		if (getSpellStack(entityplayer.getHeldItem(hand)) != null)
			entityplayer.setActiveHand(hand);

		return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, entityplayer.getHeldItem(hand));
	}
