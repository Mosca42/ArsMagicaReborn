...
public class ItemInfinityOrb extends ItemArsMagica {
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand hand) {
		SkillPoint point = SkillPointRegistry.getPointForTier(playerIn.getHeldItem(hand).getItemDamage());
		if (point == null)
			playerIn.sendMessage(new TextComponentString("Broken Item : Please use a trash bin."));
		ItemStack stack = playerIn.getHeldItem(hand);
		stack = doGiveSkillPoints(playerIn, stack, point);
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		return I18n.translateToLocal("item."+ ArsMagicaReborn.MODID +":inf_orb_" + point.toString().toLowerCase() + ".name");
	}
