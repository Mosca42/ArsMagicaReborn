...
public class SpellBase extends ItemSpellBase{
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer caster, EnumHand hand){
		if (!caster.getHeldItem(hand).hasTagCompound()) return new ActionResult<ItemStack>(EnumActionResult.FAIL, caster.getHeldItem(hand));
		if (!caster.getHeldItem(hand).hasDisplayName()){
			if (!world.isRemote)
				FMLNetworkHandler.openGui(caster, ArsMagicaReborn.instance, IDDefs.GUI_SPELL_CUSTOMIZATION, world, (int)caster.posX, (int)caster.posY, (int)caster.posZ);
		} else {
