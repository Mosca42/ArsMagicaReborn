...
public class ItemJournal extends ItemArsMagica{
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand){
		...
		if (!player.world.isRemote){
			if (getOwner(player.getHeldItem(hand)) == null){
				setOwner(player.getHeldItem(hand), player);
			}else if (!getOwner(player.getHeldItem(hand)).equals(player.getName())){
			...
			if (player.isSneaking()){
				...
				addXPToJournal(player.getHeldItem(hand), removedXP);
			}else{
				int amt = Math.min(getXPInJournal(player.getHeldItem(hand)), 10);
				if (amt > 0){
					player.addExperience(amt);
					deductXPFromJournal(player.getHeldItem(hand), amt);
				}
			}
		}
