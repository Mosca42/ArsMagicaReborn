...
public class BlockVinteumCauldron extends BlockContainer {
	...
	public BlockVinteumCauldron() {
		super(Material.IRON, MapColor.STONE);
		this.setDefaultState(this.blockState.getBaseState().withProperty(LEVEL, Integer.valueOf(0)));
		this.setUnlocalizedName("vinteum_cauldron");
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		...
		if (tile instanceof TileEntityVinteumCauldron) {
			if (!(entityIn instanceof EntityPlayer)) {
				if (entityIn instanceof EntityItem) {
					EntityItem eItem = (EntityItem) entityIn;
					if (!((TileEntityVinteumCauldron) tile).isHasDust()) {
						if (eItem.getEntityItem().getItem() == ItemDefs.itemOre && eItem.getEntityItem().getItemDamage() == 0) {
							if (getWaterLevel(worldIn, pos) == 3 || getWaterLevel(worldIn, pos) == 2 || getWaterLevel(worldIn, pos) == 1) {
								if (!worldIn.isRemote) {
									((TileEntityVinteumCauldron) tile).setHasDust(true);
								}
								entityIn.setDead();
							}
						}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (tile instanceof TileEntityVinteumCauldron) {
			...
			if (itemstack.isEmpty()) {
				...
				if (item == UniversalBucket.getFilledBucket(ForgeModContainer.getInstance().universalBucket, BlockDefs.purified_liquid_essence).getItem() && itemstack.getTagCompound().getString("FluidName").equals("purified_liquid_essence")) {
					if (i == 0 || i > 3 && i < 6 && !worldIn.isRemote) {
						if (!playerIn.capabilities.isCreativeMode) {
							playerIn.setHeldItem(hand, new ItemStack(Items.BUCKET));
						}
						this.setWaterLevel(worldIn, pos, state, 6);
						worldIn.playSound((EntityPlayer) null, pos, SoundEvents.ITEM_BUCKET_EMPTY, SoundCategory.BLOCKS, 1.0F, 1.0F);
						return true;
					}
				} else if (item == UniversalBucket.getFilledBucket(ForgeModContainer.getInstance().universalBucket, BlockDefs.liquid_essence).getItem() && itemstack.getTagCompound().getString("FluidName").equals("liquid_essence")) {
					...
					if (i == 3 && !worldIn.isRemote) {
						if (!playerIn.capabilities.isCreativeMode) {
							...
							} else if (!playerIn.inventory.addItemStackToInventory(UniversalBucket.getFilledBucket(ForgeModContainer.getInstance().universalBucket, BlockDefs.liquid_essence))) {
								playerIn.dropItem(UniversalBucket.getFilledBucket(ForgeModContainer.getInstance().universalBucket, BlockDefs.liquid_essence), false);
							}
						}
						this.setWaterLevel(worldIn, pos, state, 0);
						worldIn.playSound((EntityPlayer) null, pos, SoundEvents.ITEM_BUCKET_FILL, SoundCategory.BLOCKS, 1.0F, 1.0F);
	...
	public void setWaterLevel(World worldIn, BlockPos pos, IBlockState state, int level) {
		worldIn.setBlockState(pos, state.withProperty(LEVEL, Integer.valueOf(MathHelper.clamp(level, 0, 6))), 2);
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityVinteumCauldron();
	}
