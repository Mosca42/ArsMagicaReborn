...
public class BlockEverstone extends BlockAMPowered {
	...
	public boolean addDestroyEffects(World world, BlockPos pos, ParticleManager manager) {
		...
		for (int i = 0; i < 5 * ArsMagicaReborn.config.getGFXLevel(); ++i) {
			...
			manager.addEffect(new ParticleDigging.Factory().createParticle(0, world, pos.getX() + world.rand.nextDouble(), pos.getY() + world.rand.nextDouble(), pos.getZ() + world.rand.nextDouble(), 0, 0, 0, Block.getStateId(block)));
		}
	...
	public boolean addHitEffects(IBlockState state, World worldObj, RayTraceResult target, ParticleManager manager) {
		...
		manager.addEffect(new ParticleDigging.Factory().createParticle(0, worldObj, target.getBlockPos().getX() + worldObj.rand.nextDouble(), target.getBlockPos().getY() + worldObj.rand.nextDouble(), target.getBlockPos().getZ() + worldObj.rand.nextDouble(), 0, 0, 0, Block.getStateId(block)));
