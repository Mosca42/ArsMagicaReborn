...
public class TileEntityLectern extends TileEntityEnchantmentTable implements ITickable {
	...
	public void update() {
		if (world.isRemote) {
			updateBookRender();
			if (tooltipStack != ItemStack.EMPTY && tickCount % 4 == 0) {
				AMParticle particle = (AMParticle) ArsMagicaReborn.proxy.particleManager.spawn(world, "sparkle", pos.getX() + 0.5 + ((world.rand.nextDouble() * 0.2) - 0.1), pos.getY() + 1, pos.getZ() + 0.5 + ((world.rand.nextDouble() * 0.2) - 0.1));
				if (particle != null) {
	...
	public boolean setStack(ItemStack stack) {
		...
		if (stack == ItemStack.EMPTY) {
			...
			if (getValidItems().contains(stack.getItem())) {
				stack.setCount(1);
				this.stack = stack;
				if (!this.world.isRemote) {
					AMDataWriter writer = new AMDataWriter();
