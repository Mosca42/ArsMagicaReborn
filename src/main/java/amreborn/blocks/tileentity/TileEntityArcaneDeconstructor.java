...
public class TileEntityArcaneDeconstructor extends TileEntityAMPower implements IInventory, ISidedInventory, IKeystoneLockable<TileEntityArcaneDeconstructor>{
	...
	public void update(){
		...
		if (world.isRemote){
			if (particleCounter == 0 || particleCounter++ > 1000){
				particleCounter = 1;
				radiant = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "radiant", pos.getX() + 0.5f, pos.getY() + 0.5f, pos.getZ() + 0.5f);
				if (radiant != null){
	...
	private boolean getDeconstructionRecipe(){
		...
		if (checkStack.getItem()== ItemDefs.spell){
					...
					for (Object o : componentParts){
						...
						if (stack != ItemStack.EMPTY){
							if (stack.getItem() == ItemDefs.bindingCatalyst){
								stack.setItemDamage(((Binding)ArsMagicaAPI.getSpellRegistry().getObject(new ResourceLocation(ArsMagicaReborn.MODID, "binding"))).getBindingType(checkStack));
							}
						...
						for (Object o : componentParts){
							...
							if (stack != ItemStack.EMPTY){
								if (stack.getItem() == ItemDefs.crystalPhylactery){
									ItemDefs.crystalPhylactery.setSpawnClass(stack,((Summon)ArsMagicaAPI.getSpellRegistry().getObject(new ResourceLocation(ArsMagicaReborn.MODID, "summon"))).getSummonType(checkStack));
			...
			if (recipeParts != null && checkStack != null && recipe.getRecipeOutput() != ItemStack.EMPTY){
				...
				for (Object o : recipeParts){
					ItemStack stack = objectToItemStack(o);
					if (stack != ItemStack.EMPTY && !stack.getItem().hasContainerItem(stack)){
						stack.setCount(1);
						recipeItems.add(stack.copy());
					}
				}
	...
	public ITextComponent getDisplayName() {
		return new TextComponentString("Arcane Deconstructor");
	}
