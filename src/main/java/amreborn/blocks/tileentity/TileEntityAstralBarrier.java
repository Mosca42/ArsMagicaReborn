...
public class TileEntityAstralBarrier extends TileEntityAMPower implements IInventory, IKeystoneLockable<TileEntityAstralBarrier>{
	...
	public int getRadius(){
		if (this.inventory.get(0) != ItemStack.EMPTY && this.inventory.get(0).getItem() instanceof ISpellFocus){
	...
	public void update(){
		...
		if (world.isRemote){
			if (IsActive()){
				if (displayAura){
					AMParticle effect = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "symbols", pos.getX(), pos.getY() + 0.5, pos.getZ());
					if (effect != null){
				...
				if (particleTickCounter >= 15){
					...
					AMParticle effect = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "sparkle", pos.getX() + 0.5, pos.getY() + 0.1 + world.rand.nextDouble() * 0.5, pos.getZ() + 0.5);
					if (effect != null){
						effect.setIgnoreMaxAge(false);
						effect.setMaxAge(100);
						effect.setParticleScale(0.5f);
...
//	}
	...
	public ITextComponent getDisplayName() {
		...
		return new TextComponentString("Astral Barrier");
	}
