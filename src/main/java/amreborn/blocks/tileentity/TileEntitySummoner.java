...
public class TileEntitySummoner extends TileEntityAMPower implements IInventory, IKeystoneLockable<TileEntitySummoner> {
	...
	private static final int powerPadding = 500; // extra power to charge before summoning so that it can be maintained for a
													// while
	...
	public void update() {
		...
		if (!world.isRemote) {
			...
			if (isRedstonePowered() && inventory.get(SUMMON_SLOT) != ItemStack.EMPTY) {
	...
	private void summonCreature() {
		if (world.isRemote || this.summonEntityID != -1)
			return;
		if (dummyCaster == null) {
			dummyCaster = new DummyEntityPlayer(world);
		}
		EntityLiving summon = ((Summon) ArsMagicaAPI.getSpellRegistry().getObject(new ResourceLocation(ArsMagicaReborn.MODID + ":summon"))).summonCreature(inventory.get(SUMMON_SLOT), dummyCaster, dummyCaster, world, pos.getX(), pos.getY() + 1, pos.getZ());
		if (summon != null) {
