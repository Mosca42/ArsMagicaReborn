...
public class TileEntityEverstone extends TileEntity implements ITickable{
	...
	private static final int reconstructMax = ArsMagicaReborn.config.getEverstoneRepairRate();
	...
	private void propagatePoweredByEverstone(boolean powered, ArrayList<BlockPos> completedUpdates){
		...
		if (world.isRemote){
			AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "radiant", pos.getX() + 0.5f, pos.getY() + 0.5f, pos.getZ() + 0.5f);
			if (particle != null){
		...
		if (world.isBlockIndirectlyGettingPowered(pos) == 0 && !poweredFromEverstone){
			...
			if (world.isRemote){ //
				...
				if (reconstructTimer < reconstructMax - 20 && reconstructTimer > 20 && world.rand.nextInt(10) < 8){
					//TODO ArsMagicaReborn.proxy.addDigParticle(worldObj, pos, getFacade() == null ? BlockDefs.everstone : getFacade(), getFacadeMeta());
				}
			}
		}
