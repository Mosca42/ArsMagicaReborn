...
//		sunstone.put(0, BlockDefs.blocks.getDefaultState().withProperty(BlockArsMagicaBlock.BLOCK_TYPE, BlockArsMagicaBlock.EnumBlockType.SUNSTONE));
	...
	public void update(){
		...
		if (isCrafting){
			...
			ArsMagicaReborn.proxy.particleManager.RibbonFromPointToPoint(world, pos.getX() + 0.5, pos.getY() - 2, pos.getZ() + 0.5, pos.getX() + 0.5, pos.getY() - 3, pos.getZ() + 0.5);
			}
			...
			for (EntityItem item : components){
				...
				if (stack != ItemStack.EMPTY && compareItemStacks(stack, entityItemStack)){
					if (!world.isRemote){
						updateCurrentRecipe(item);
						item.setDead();
					}else{
						//TODO worldObj.playSound(pos.getX(), pos.getY(), pos.getZ(), ArsMagicaReborn.MODID + ":misc.craftingaltar.component_added", 1.0f, 0.4f + worldObj.rand.nextFloat() * 0.6f, false);
						for (int i = 0; i < 5 * ArsMagicaReborn.config.getGFXLevel(); ++i){
							AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(world, "radiant", item.posX, item.posY, item.posZ);
							if (particle != null){
	...
	private void updatePowerRequestData(){
		ItemStack stack = getNextPlannedItem();
		if (stack != ItemStack.EMPTY && stack.getItem().equals(ItemDefs.etherium)){
			if (switchIsOn()){
		...
		if (items.size() == 1){
			...
			if (item != null && !item.isDead && item.getEntityItem() != null && item.getEntityItem().getItem() == ItemDefs.spellParchment){
				if (!world.isRemote){
					...
					if (getNextPlannedItem() == ItemStack.EMPTY || getNextPlannedItem().getItem() != ItemDefs.spellParchment)
					...
					//worldObj.playSound(pos.getX(), pos.getY(), pos.getZ(), ArsMagicaReborn.MODID + ":misc.craftingaltar.create_spell", 1.0f, 1.0f, true);
				}
			}
		}
	}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound nbttagcompound){
		...
		if (addedPhylactery != ItemStack.EMPTY){
		...
		if (addedBindingCatalyst != ItemStack.EMPTY){
