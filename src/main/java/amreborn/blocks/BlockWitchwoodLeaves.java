...
import amreborn.particles.ParticlePendulum;
...
public class BlockWitchwoodLeaves extends BlockLeaves{
	...
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		if (!ArsMagicaReborn.config.witchwoodLeafPFX())
			return;
		if (rand.nextInt(300) == 0 && worldIn.isAirBlock(pos.down())){
			AMParticle particle = (AMParticle)ArsMagicaReborn.proxy.particleManager.spawn(worldIn, "leaf", pos.getX() + rand.nextDouble(), pos.getY() + rand.nextDouble(), pos.getZ() + rand.nextDouble());
			if (particle != null){
