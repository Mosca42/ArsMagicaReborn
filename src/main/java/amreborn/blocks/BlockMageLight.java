...
import amreborn.particles.ParticleGrow;
...
public class BlockMageLight extends BlockAMSpecialRender {
	...
	public int tickRate(World par1World) {
		return 20 - 5 * ArsMagicaReborn.config.getGFXLevel();
	}

	@Override
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		AMParticle particle = (AMParticle) ArsMagicaReborn.proxy.particleManager.spawn(worldIn, "sparkle", pos.getX() + 0.5 + (rand.nextDouble() * 0.2f - 0.1f), pos.getY() + 0.5, pos.getZ() + 0.5 + (rand.nextDouble() * 0.2f - 0.1f));
		if (particle != null) {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!worldIn.isRemote && playerIn.getHeldItem(hand) != ItemStack.EMPTY) {
